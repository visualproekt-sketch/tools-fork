...
def get_tool_verts_curv(lw_tool: MI_Linear_Widget, verts_ids: List[Any], obj: bpy.types.Object, do_clamp: bool, local_coords: bool) -> List[Tuple[Any, float, Vector]]:
    apply_tool_verts = []
    final_dir = ( lw_tool.end_point.position - lw_tool.start_point.position )
    max_dist = final_dir.length
    for point in verts_ids:
        v_pos = obj.matrix_world @ point.co.xyz
        value = mathu.geometry.distance_point_to_plane(v_pos, lw_tool.start_point.position, final_dir)
        if value > 0:
            if value > max_dist and do_clamp:
                value = 1.0
            ...
            pos_final = v_pos
            if local_coords is True:
                pos_final = point.co.xyz

            apply_tool_verts.append( (point, value, pos_final) )
